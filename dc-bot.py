...
async def error(ctx):
    try:
        # Bu bir hata yükseltecek
        1 / 0
    except Exception as e:
        # Hatanın izini sür ve bir metin dosyasına yaz
        error_traceback = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
        with open(log_klasörü + 'error_log.txt', 'w') as f:
            f.write(error_traceback)
        await ctx.send('Bir hata oluştu. Hata kaydı error_log.txt dosyasına kaydedildi.')
